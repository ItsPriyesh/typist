...
class WordRenderer(containerSelector: String, strings: Array[String]) extends Observer[Result[String]] {
  ...
  var index = 0
  var renderCache: Array[String] = strings.map(toSpan(_, Default))
  render()
  ...
  override def onNext(result: Result[String]): Future[Ack] = {
    renderCache(index) = result match {
      case Success(e) => println("success " + e); toSpan(e, Correct);
      case Failure(e, a) => println("failure " + e + " " + a); toSpan(e, Incorrect)
    }
